...
int main() {
    ...
    int foo = 1, bar = 2;
    Swap(foo, bar);
    SimpleContainer<std::string> demoContainer;
    demoContainer.Add("Barbie");
    Logger::Log("Template function and class used at startup.");
    ...
    if (background.id == 0) {
        Logger::Log("Failed to load pink.jpg");
        CloseWindow();
        return 1;
    }
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_F5)) {
            game.session.SaveState("savefile.dat");
            Logger::Log("Game state saved by user.");
        }
        if (IsKeyPressed(KEY_F9)) {
            game.session.LoadState("savefile.dat");
            Logger::Log("Game state loaded by user.");
        }
        ...
        case SPLASH:
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (splash.ShouldStartGame()) {
                    ...
                    game.currentLevel = 1;
                    state = LEVEL_1;
                    Logger::Log("Entered LEVEL_1 from SPLASH.");
                }
            }
            break;
        ...
        case LEVEL_1: {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (canEnterShow && CheckCollisionPointRec(mouse, finalButton)) {
                    ...
                    level2Screen->Load();
                    state = SCREEN_2;
                    Logger::Log("Entered SCREEN_2 from LEVEL_1.");
                }
            }
            break;
        }
        ...
        case SCREEN_2:
            if (level2Screen) level2Screen->Draw();
            if (level2Screen && level2Screen->ShouldContinue()) {
                delete level2Screen;
                ...
                game.currentLevel = 2;
                state = LEVEL_2;
                Logger::Log("Entered LEVEL_2 from SCREEN_2.");
            }
            break;

        case LEVEL_2: {
            if (level2) level2->Draw();
            game.DrawUI();
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (game.session.GetInventory().Size() == 2 && CheckCollisionPointRec(mouse, nextButton)) {
                    ...
                    Logger::Log("Entered SCREEN_4 from LEVEL_2.");
                }
            }
            break;
        }
        ...
        case SCREEN_4:
            if (waitForKeyRelease) {
                ...
                if (screen4 && screen4->ShouldContinue()) {
                    ...
                    state = SPLASH;
                    Logger::Log("Returned to SPLASH after final fashion show.");
                }
            }
            break;
